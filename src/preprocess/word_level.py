...
def spell_correction(df):
    """
    Function that when applied to a dataframe transforms its 'processed_tweet' column
    to correct the spelling of words in its sentences.

    :param df: a dataframe that the change of the laughing patterns is going to be applied to
    """
    df['spell_correction'] = df['processed_tweet'].progress_apply(lambda x: ' '.join([Word(w).correct() for w in x.split()]))
...
def normalize_repeating_characters(df):
    """
    Function that when applied to a dataframe transforms its 'processed_tweet' column
    to normalize repetitions of the same character into a single instance of the character.

    :param df: a dataframe that the change of the laughing patterns is going to be applied to
    """
    repetition_pattern = re.compile(r'([a-z])\1{3,}')
    df['normalize_rep_chars'] = df['processed_tweet'].progress_apply(lambda x: repetition_pattern.sub(r'\1', x))
    df['processed_tweet'] = df['normalize_rep_chars'].copy()
...
def segment_hashtags(df):
    """
    Function that when applied to a dataframe transforms its 'processed_tweet' column
    to segment the hashtags in its sentences.

    :param df: a dataframe that the change of the laughing patterns is going to be applied to
    """
    df['segment_hashtags'] = df['processed_tweet'].progress_apply(lambda x: ' '.join([wordsegment.segment(w[1:]) if w.startswith('#') else w for w in x.split()]))
    df['processed_tweet'] = df['segment_hashtags'].copy()


def reconstruct_smileys(df):
    """
    Function that when applied to a dataframe transforms its 'processed_tweet' column
    to reconstruct the smileys in its sentences.
    ...
    df['reconstruct_smileys'] = df['processed_tweet'].progress_apply(lambda x: rs(x))
    df['processed_tweet'] = df['reconstruct_smileys'].copy()
...
def apply_word_level(df, cfg):
    ...
    if cfg.LEMMATIZE:
        ...
        time.sleep(2)
        lemmatize(df)
    if cfg.SPELL_CORRECTION:
        print(">>> Spelling correction")
        time.sleep(2)
        spell_correction(df)
    if cfg.NORMALIZE_REPEATING_CHARACTERS:
        print(">>> Normalizing repeating characters")
        time.sleep(2)
        normalize_repeating_characters(df)
    if cfg.REMOVE_NUMERIC:
        print(">>> Removing numeric words")
        time.sleep(2)
        remove_numeric(df)
    if cfg.RECONSTRUCT_SMILEYS:
        print(">>> Reconstructing smileys")
        time.sleep(2)
        reconstruct_smileys(df)
