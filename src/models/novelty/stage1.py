...
from src.models.utils import numpy_softmax, tokenization, compute_metrics, save_test_preds, get_wrongly_pred_samples, sort_by_difficulty, keep_distribution
from src.models.io_handler import load_testing_tweets_to_df, load_from_pkl, save_to_pkl, save_logits


def train_stage1_split(cfg, split_number):
    """
    A function that given a config is able to train the split number run of the second stage
    ...
    train_dataset_sorted = sort_by_difficulty(train_val_folds[0][0], cfg.IO.SCORES_DICTIONARY_PATH, tokenizer, cfg.PARAMETERS.TOK_MAX_LENGTH)
    train_dataset_same_dist = keep_distribution(train_dataset_sorted, tokenizer, cfg.PARAMETERS.TOK_MAX_LENGTH)

    evaluation_dataset_sorted = sort_by_difficulty(train_val_folds[0][1], cfg.IO.SCORES_DICTIONARY_PATH, tokenizer, cfg.PARAMETERS.TOK_MAX_LENGTH)
    ...
    X_val_false, y_val_false = get_wrongly_pred_samples(train_val_folds[0][1]['tweet'],
                                                        y_preds_val,
                                                        train_val_folds[0][1]['label'])

    val_data_false = {"tweet": X_val_false, "label": y_val_false}
    val_false_dataset = Dataset.from_dict(val_data_false)
    val_false_tokenized_dataset = val_false_dataset.map(lambda examples: tokenization(tokenizer, examples, cfg.PARAMETERS.TOK_MAX_LENGTH), batched=True)

    save_to_pkl(val_false_tokenized_dataset, f'val_false_tokenized_dataset_stage1_split_{split_number}.pkl')
    save_test_preds(y_preds_test, test_results_path + f"test_data_stage1_split_{split_number}.csv")
    save_logits(numpy_softmax(results_val.predictions), test_results_path, cfg.PARAMETERS.MODEL_NAME, f'logits_val_stage1_split_{split_number}.txt')
    save_logits(numpy_softmax(results_test.predictions), test_results_path, cfg.PARAMETERS.MODEL_NAME, f'logits_test_stage1_split_{split_number}.txt')
    return test_results_path + f'logits_test_stage1_split_{split_number}.txt'
